...
func Handler(ctx context.Context, event events.KinesisEvent) error {
	...
	rssFeed := rss.NewFeed()
	newsItems, err := rssFeed.ScrapeAWSNews(ctx, cfg.RSSFeed.URL)
	if err != nil {
		log.Logger.Fatal().Err(err).Msg("error parsing RSS feed")
	}

	db, err := dynamodb.NewDynamoDB(cfg.DynamoDB.TableName)
	if err != nil {
		log.Logger.Fatal().Err(err).Msg("error creating DynamoDB client")
	}

	bsky, err := bluesky.NewBluesky(cfg.Bluesky.Handle, cfg.Bluesky.Password)
	if err != nil {
		log.Logger.Fatal().Err(err).Msg("error creating Bluesky client")
	}
